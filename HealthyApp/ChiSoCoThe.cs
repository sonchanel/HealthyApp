...
namespace HealthyApp
{
	public partial class ChiSoCoThe : Form
	{
		...
		string Tk;
		public ChiSoCoThe(string taikhoan)
		{
			Tk = taikhoan;
			InitializeComponent();
			KiemtraNgaysinh();
		...
		private void btTinhtoan_Click(object sender, EventArgs e)
		{
			...
			if (tuoi < 50 && rbtNam.Checked == true)
			{
				if (BMI <= 18.5)
				{
			...
			else if (tuoi < 50 && rbtNu.Checked == true)
			{
				if (BMI <= 18)
				{
					lbDanhgia.Text = "Nhẹ cân";
		...
		{
			string truyvan = string.Format("select Ngaysinh from Taikhoan where Taikhoan = '{0}'"
				,Tk);
			...
			if (db.Rows.Count > 0)
			{
				...
				if (a != null)
				{
					dateNgaysinh.Value = DateTime.Parse(a);
				}
				else
				{
